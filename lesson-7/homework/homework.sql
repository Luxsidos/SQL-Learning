...
-- 4
select count(distinct Category) as count_of_unique_categories from Products

-- 5
select sum(Price * StockQuantity) as total_amount from Products
where ProductID = 7
...
-- 7
select DepartmentName, Count(*) as count_of_employees from Employees
...
select 
	ProductID, 
	Sum(SaleAmount) as sum_sale_amount,
...
-- 16
select ProductID, avg(SaleAmount) as avg_amount from Sales
group by ProductID
having avg(SaleAmount) > 400

-- 17
select Year(SaleDate) as year, sum(SaleAmount) as total_amount from Sales
...
select ProductID, avg(SaleAmount) as Avg_amount from Sales
group by ProductID
having avg(SaleAmount) > 150

-- 21
select 
	CustomerID, sum(SaleAmount) as total_amount 
from Sales
group by CustomerID
...
-- 24
select YEAR(OrderDate), sum(TotalAmount * Quantity) as totalAmount, count(Distinct ProductID) as ProductID from Orders 
group by YEAR(OrderDate)
having Sum(Quantity) > 2

-- 25

select YEAR(OrderDate), max(Quantity) as maxQuantity, min(Quantity) as minQuantity from Orders 
group by YEAR(OrderDate)