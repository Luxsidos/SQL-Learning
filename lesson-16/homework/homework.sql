...
;WITH Factorial AS (
    ...
    FROM Numbers1
    UNION ALL
    SELECT n.Number, f.Fact * (f.Step + 1), f.Step + 1
    FROM Numbers1 n
    JOIN Factorial f ON n.Number = f.Number AND f.Step < n.Number
)
...
Diffs AS (
    SELECT Y, M, TotalSales,
           LAG(TotalSales) OVER (ORDER BY Y, M) AS PrevSales
    FROM MonthlySales
)
SELECT Y, M, TotalSales, PrevSales, TotalSales - PrevSales AS Diff
FROM Diffs;
...
;WITH Fibonacci AS (
    SELECT 1 AS n, CAST(0 AS int) AS a, CAST(1 AS int) AS b
    UNION ALL
    SELECT n + 1, b, a + b
    FROM Fibonacci
    WHERE n < @maxN
...
DECLARE @n INT = 5;
;WITH Seq AS (
    SELECT CAST('1' AS VARCHAR(50)) AS num, 1 AS step
    UNION ALL
    SELECT num + CAST(step+1 AS VARCHAR(10)), step+1
    FROM Seq
    WHERE step < @n
)
SELECT * FROM Seq
