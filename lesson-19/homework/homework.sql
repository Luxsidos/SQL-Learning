...
-- 1. Creates a temp table #EmployeeBonus
-- 2. Inserts EmployeeID, FullName, Department, Salary, and BonusAmount
--    (BonusAmount = Salary * BonusPercentage / 100)
-- 3. Selects all data from the temp table
------------------------------------------------------
...
-- Updates salary of all employees in that department by the percentage
...
-- Deletes products missing in Products_New
...
-- Task 4:
-- Tree Node
-- Each node can be "Leaf", "Root", or "Inner"
...
-- If no requests exist, rate should be 0
------------------------------------------------------
...
CREATE TABLE employees_lowest (
    id INT PRIMARY KEY,
    name VARCHAR(100),
    salary DECIMAL(10,2)
);

INSERT INTO employees_lowest (id, name, salary) VALUES
(1, 'Alice', 50000),
(2, 'Bob', 60000),
(3, 'Charlie', 50000);

SELECT *
FROM employees_lowest
WHERE salary = (SELECT MIN(salary) FROM employees_lowest);
...
-- Create a stored procedure that:
-- Accepts @ProductID
-- Returns: ProductName, Total Quantity Sold, Total Sales Amount,
