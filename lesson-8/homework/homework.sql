...
select 
	Category, 
	count(StockQuantity) as CountOfProducts 
from Products
group by Category
having count(StockQuantity) > 0
...
-- 7 
select *, 
	IIF(StockQuantity < 30, 'Low Stock', 'Sufficient') as QuantityStatus
from Products
...
-- 13
select year(OrderDate) as year, avg(TotalAmount) as totalAmount from Orders
...
select * INTO Population_Each_City from 
...
-- 21
select p.ProductName, sum(s.SaleAmount) as totalAmount from Sales s join Products p
