...
-- EASY 1
-- Write an SQL query to split the Name column into two separate columns: one for the name and one for the surname. (TestMultipleColumns table)
SELECT 
    Id,
...
-- EASY 2
-- Write an SQL query to find all strings that contain the % symbol. (TestPercent table)
SELECT *
FROM TestPercent
WHERE Strs LIKE '%[%]%';

-- EASY 3
-- Write an SQL query to split the string based on the dot symbol (.) into two separate columns. (Splitter table)
SELECT 
    Id,
...
-- EASY 4
-- Write an SQL query to return rows that have more than 2 dots in the string. (testDots table)
SELECT *
FROM testDots
WHERE LEN(Vals) - LEN(REPLACE(Vals, '.', '')) > 2;

-- EASY 5
-- Write an SQL query to count the number of spaces in each string. (CountSpaces table)
SELECT 
    texts,
...
-- EASY 6
-- Write an SQL query to find employees who earn more than their managers. (Employee table)
...
-- EASY 7
-- Write an SQL query to find employees with between 10 and 15 years of service. (Employees table)
SELECT 
    EMPLOYEE_ID,
    FIRST_NAME,
    LAST_NAME,
...
-- MEDIUM 1
-- Write an SQL query to find all dates where the temperature was higher than the previous day. (weather table)
...
-- MEDIUM 2
-- Write an SQL query to find the first login date for each player. (Activity table)
SELECT player_id, MIN(event_date) AS FirstLoginDate
FROM Activity
GROUP BY player_id;

-- MEDIUM 3
-- Write an SQL query to extract the third item from the fruit list. (fruits table)
;WITH CTE AS (
    SELECT 
        f.Id,
        value,
        ROW_NUMBER() OVER (PARTITION BY f.Id ORDER BY (SELECT NULL)) AS rn
    FROM fruits f
    CROSS APPLY STRING_SPLIT(fruit_list, ',')
)
SELECT Id, value AS ThirdFruit
FROM CTE
WHERE rn = 3;

-- MEDIUM 4
-- Write an SQL query to categorize employees into different employment stages based on their years of service. (Employees table)
SELECT 
    EMPLOYEE_ID,
    FIRST_NAME,
    LAST_NAME,
...
-- MEDIUM 5
-- Write an SQL query to extract the integer at the start of each string. (GetIntegers table)
SELECT 
    Id,
...
-- DIFFICULT 1
-- Write an SQL query to swap the first two letters in a comma-separated string. (MultipleVals table)
SELECT 
    Id,
    STUFF(Vals, 1, 1, SUBSTRING(Vals, 3, 1)) AS SwappedVals
FROM MultipleVals;

-- DIFFICULT 2
-- Write an SQL query to convert a string into rows of single characters. Example: 'sdgfhsdgfhs@121313131' (no table, use variable)
...
SELECT SUBSTRING(@str, n, 1) AS CharVal
FROM Numbers;

-- DIFFICULT 3
-- Write an SQL query to find the first device used by each player. (Activity table)
...
-- DIFFICULT 4
-- Write an SQL query to separate integers and characters from the given string 'rtcfvty34redt'. (no table, use variable)
DECLARE @str2 VARCHAR(100) = 'rtcfvty34redt';

;WITH CTE AS (
    SELECT 1 AS n, SUBSTRING(@str2,1,1) AS ch
    UNION ALL
    SELECT n+1, SUBSTRING(@str2,n+1,1)
    FROM CTE
    WHERE n < LEN(@str2)
)
SELECT 
