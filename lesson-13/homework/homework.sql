...
-- 4
select MANAGER_ID, sum(SALARY) as Salary from Employees
group by MANAGER_ID

-- 5
select * from TestMax
select Year1, MAX(Max1) as max1, MAX(Max2) max2, MAX(Max3) max3
from TestMax
GROUP BY Year1
...
-- 8
select COALESCE(ssn, passportid, itin) as non_null_or_null from person

-- medium
-- 1
select 
	parsename(REPLACE(FullName, ' ', '.'), 3) FirstName,
...
SELECT 
    ...
    STRING_AGG(CASE WHEN ch LIKE '[A-Za-z0-9]' THEN '' ELSE ch END, '') AS OtherChars
FROM Chars;
